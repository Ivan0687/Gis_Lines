...
CREATE TABLE IF NOT EXISTS `lines`.`junctions_has_fittings` (
  ...
  `fittings_id` INT NOT NULL,
  PRIMARY KEY (`junctions_id`, `fittings_id`),
  INDEX `fk_junctions_has_fittings_fittings1_idx` (`fittings_id` ASC),
  INDEX `fk_junctions_has_fittings_junctions1_idx` (`junctions_id` ASC),
...
CREATE TABLE IF NOT EXISTS `lines`.`junctions_has_insulators` (
  `junctions_id` INT NOT NULL,
  `insulators_id` INT NOT NULL,
  PRIMARY KEY (`junctions_id`, `insulators_id`),
  INDEX `fk_junctions_has_insulators_insulators1_idx` (`insulators_id` ASC),
  INDEX `fk_junctions_has_insulators_junctions1_idx` (`junctions_id` ASC),
...
CREATE TABLE IF NOT EXISTS `lines`.`towers` (
  ...
  `center_gps_coordinates_id` INT NOT NULL,
  PRIMARY KEY (`id`),
  UNIQUE INDEX `id_UNIQUE` (`id` ASC),
  INDEX `fk_tower_tower_marks1_idx` (`tower_mark_id` ASC),
  INDEX `fk_tower_gps_coordinates1_idx` (`center_gps_coordinates_id` ASC),
  ...
  CONSTRAINT `fk_tower_gps_coordinates1`
    FOREIGN KEY (`center_gps_coordinates_id`)
    REFERENCES `lines`.`gps_coordinates` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
